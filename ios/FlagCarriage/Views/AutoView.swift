                ...
                VStack(spacing: 20) {
                    ...
                    case .set:
                        SetRunPanel(
                            ...
                            onStart: {
                                // Run each run in the set sequentially
                                if let set = selectedSet, let firstRunID = set.entries.first?.runID,
                                   let firstRun = store.run(for: firstRunID) {
...
struct CattleSimPanel: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            ProfileDetailCard(profile: selectedProfile, onEdit: onEditProfile)
                .padding(.horizontal)
        ...
        VStack(spacing: 16) {
            if sets.isEmpty {
                VStack(spacing: 8) {
                    ...
                    Text("No training sets yet")
                        .foregroundColor(.secondary)
                    Text("Create sets in the Program tab")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                .padding()
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    ForEach(sets) { set in
                        SetCard(set: set, isSelected: set.id == selectedSet?.id, store: store)
                            .padding(.horizontal)
                            .onTapGesture { selectedSet = set }
                    }
                }

                BigActionButton(
                    isRunning: runEngine.isRunning,
                    startLabel: "Run Set",
                    stopLabel: "Stop",
                    onStart: onStart,
                    onStop: onStop
...
struct LiveStatusCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if mode == .random {
                Text(simEngine.currentBehaviour)
                ...
                Text(String(format: "%.1fs remaining", runEngine.timeRemaining))
                    .font(.caption).foregroundColor(.secondary)
            }
        }
        .padding()
...
struct ProfileCard: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            Text(aggressionEmoji)
                .font(.system(size: 32))
            Text(profile.name)
                .font(.caption.weight(.semibold))
                .multilineTextAlignment(.center)
            Text(profile.aggression.label)
                .font(.caption2)
                .foregroundColor(.secondary)
        }
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .stroke(isSelected ? aggressionColor : Color.clear, lineWidth: 2)
        )
        .cornerRadius(14)
    }

    var aggressionEmoji: String {
        switch profile.aggression {
        case .lazy:   return "🐄"
        case .medium: return "🐂"
        case .hot:    return "🐃"
        }
