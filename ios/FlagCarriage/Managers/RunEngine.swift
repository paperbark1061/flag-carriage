...
// Executes a CarriageRun or a TrainingSet step-by-step,
// sending commands to the ConnectionManager.

class RunEngine: ObservableObject {
    @Published var isRunning = false
    @Published var currentStepIndex = 0
    @Published var currentRunName = ""
    @Published var progress: Double = 0    // 0-1 within current step
...
class CattleSimEngine: ObservableObject {
    ...
    private func scheduleNextPhase() {
        ...
        let doPause = Double.random(in: 0...1) < profile.pauseChance
        ...
        if doPause {
            ...
            // Pick direction — bias toward keeping current but allow reversal
            let flip = Double.random(in: 0...1) < 0.45
            let newDir: StepDirection
            if flip {
                newDir = lastDirection == .forward ? .backward : .forward
            } else {
            ...
            let dirLabel = newDir == .forward ? "Running forward" : "Cutting back"
            currentBehaviour = "\(dirLabel) @ \(Int(Double(speed)/255*100))%"
            targetDuration = duration
