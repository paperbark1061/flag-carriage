...
class ConnectionManager: ObservableObject {
    ...
    func send(_ command: String) {
        ...
        let msg = URLSessionWebSocketTask.Message.string(command)
    ...
    func autoMode() { send("A") }
    func manualMode() { send("M") }
    ...
    private func parseStatus(_ text: String) {
        // STATUS:F,SPD:200,LA:0,LB:0,IP:192.168.1.1
        ...
        for part in parts {
            ...
            switch kv[0] {
            case "STATUS": s.direction = String(kv[1])
        ...
        if let first = parts.first, first.count == 1 {
            s.direction = String(first)
        }
        lastStatus = s
    }
