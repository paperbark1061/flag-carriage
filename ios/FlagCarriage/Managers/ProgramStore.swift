...
struct SetEntry: Identifiable, Codable {
    var id = UUID()
    var runID: UUID
    var restDuration: Double   // seconds before next run
}
...
struct CattleProfile: Identifiable, Codable {
    ...
    var pauseChance: Double     // 0-1 probability of pausing
    var minPauseDuration: Double
    var maxPauseDuration: Double
    var changeFrequency: Double  // seconds between potential direction changes
    ...
    static let defaults: [CattleProfile] = [
        CattleProfile(
            ...
            pauseChance: 0.4,
            minPauseDuration: 0.8, maxPauseDuration: 2.5,
            changeFrequency: 3.0
        ),
        CattleProfile(
            name: "Medium Cow",
            ...
            pauseChance: 0.25,
            minPauseDuration: 0.3, maxPauseDuration: 1.2,
            changeFrequency: 1.8
        ),
        CattleProfile(
            name: "Hot Cow",
            ...
            pauseChance: 0.1,
            minPauseDuration: 0.2, maxPauseDuration: 0.6,
            changeFrequency: 0.9
        )
    ]
}
...
class ProgramStore: ObservableObject {
    ...
    func deleteRun(_ run: CarriageRun) {
        ...
        for i in sets.indices {
            sets[i].entries.removeAll { $0.runID == run.id }
        }
        persist()
    }

    func run(for id: UUID) -> CarriageRun? {
        runs.first { $0.id == id }
    }
    ...
    func saveProfile(_ profile: CattleProfile) {
        if let idx = cattleProfiles.firstIndex(where: { $0.id == profile.id }) {
            cattleProfiles[idx] = profile
        } else {
            cattleProfiles.append(profile)
        }
        persist()
    }
    ...
    private func load() {
        if let d = UserDefaults.standard.data(forKey: runsKey),
           let r = try? JSONDecoder().decode([CarriageRun].self, from: d) { runs = r }
        if let d = UserDefaults.standard.data(forKey: setsKey),
           let s = try? JSONDecoder().decode([TrainingSet].self, from: d) { sets = s }
        if let d = UserDefaults.standard.data(forKey: profilesKey),
           let p = try? JSONDecoder().decode([CattleProfile].self, from: d) { cattleProfiles = p }
